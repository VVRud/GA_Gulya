...
class TournamentWithoutReturn(TournamentBase):
    ...
    def select(
        ...
        parents_indices = np.empty(
            (num_copies * num_parents_in_tournament,), dtype=np.int64
        )
        for i in range(num_copies):
            ...
            parents_indices[
                i * num_parents_in_tournament : (i + 1) * num_parents_in_tournament
            ] = self.get_tournament_winner(tournament_indices, tournament_fitness)
