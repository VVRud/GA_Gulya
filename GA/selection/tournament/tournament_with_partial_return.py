...
class TournamentWithPartialReturn(TournamentBase):
    ...
    def select(self, last_generation_fitness: np.ndarray[np.float64], num_parents_mating: int, ga_instance: pygad.GA) -> Tuple[np.ndarray[np.int64, np.int64], np.ndarray[np.int64]]:
        ...
        """
        fitness_indices = np.repeat(np.arange(last_generation_fitness.shape[0]), self.tournament_size)
        ...
        parents_indices = self.get_tournament_winner(tournaments_indices, tournament_fitness)
