...
class SUSSelection(BaseWithProbabilities):
    """
    Stochastic Universal Sampling (SUS) is a selection method used in genetic algorithms.
    ...
    def select(self, last_generation_fitness: np.ndarray[np.float64], num_parents_mating: int, ga_instance: pygad.GA) -> Tuple[np.ndarray[np.int64, np.int64], np.ndarray[np.int64]]:
        ...
        pointers = [start_pointer + i / num_parents_mating for i in range(num_parents_mating)]
