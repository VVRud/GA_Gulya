...
class StopCriteria:
    ...
    def fitness_change(self, population_number: int) -> float | None:
        """
        ...
        previous_fitness = self.history_data.avg_fitness_history[
            population_number - self.fitness_change_history_length
        ]
        return abs(current_fitness - previous_fitness)

    def stop_condition(self, pygad_instance: pygad.GA) -> str | None:
        """
        ...
        if generation > self.max_generations:
            pygad_instance.logger.info(
                f"Hard stop: max generations reached "
        ...
        if (
            fitness_change is not None
            and fitness_change < self.fitness_change_threshold
        ):
            pygad_instance.logger.info(
                f"Convergence condition met: fitness change reached "
                f"({fitness_change} < {self.fitness_change_threshold})"
            )
        ...
        if self.is_gomogenic(generation):
            pygad_instance.logger.info(
                f"Convergence condition met: gomogenity reached "
                f"({self.is_gomogenic(generation)})"
            )
            self.is_converged = True
            return self.STOP_WORD
