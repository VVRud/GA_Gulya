...
def test_initialization(gray_encoder_basic, gray_encoder_decimal):
    ...
    assert gray_encoder_decimal.num_bits == 10  # -5 to 5 with 2 decimal places: 10*100 = 1000 values, need 10 bits
...
def test_binary_to_gray_conversion(gray_encoder_for_bits):
    ...
    for i in range(8):
        ...
        assert encoded == expected_gray_for_0_to_7[i], f"Gray code for {i} doesn't match expected"
...
def test_hamming_distance_property():
    ...
    for i in range(15):  # Test for values 0 to 14 (comparing with next value)
        ...
        hamming_distance = sum(bit1 != bit2 for bit1, bit2 in zip(encoded_current, encoded_next))
        
        # Gray code property: adjacent values should differ by exactly one bit
        assert hamming_distance == 1, f"Gray codes for {i} and {i+1} don't differ by exactly one bit" 