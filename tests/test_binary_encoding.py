...
def test_initialization(binary_encoder_basic, binary_encoder_decimal):
    ...
    assert binary_encoder_decimal.num_bits == 10  # -5 to 5 with 2 decimal places: 10*100 = 1000 values, need 10 bits
...
def test_decode_single_value(binary_encoder_basic, binary_encoder_decimal):
    ...
    decoded = binary_encoder_decimal.decode([0, 1, 1, 1, 1, 1, 0, 1, 0, 0])  # Binary for 500
    assert len(decoded) == 1
...
def test_decode_multiple_values(binary_encoder_basic):
    ...
    decoded = binary_encoder_basic.decode([0, 0, 0, 1, 0, 1, 1, 1])  # Binary for 1 and 7
...
def test_encode_decode_roundtrip():
    ...
    for original, result in zip(original_values, decoded):
...
def test_all_values_in_range(value):
    ...
    assert encoded == [int(v) for v in format(value + 1024, 'b').zfill(encoder.num_bits)], f"Failed for value {to_encode}"
    assert pytest.approx(encoder.decode(encoded), 0.01) == [to_encode], f"Failed for value {to_encode}"
...
def test_all_values_in_range_cutoff(value):
    ...
    encoded = encoder.encode([to_encode])
    assert encoded == [int(v) for v in format(value + 102, 'b').zfill(encoder.num_bits)], f"Failed for value {to_encode}"
    assert encoder.decode(encoded) == pytest.approx([to_encode], 0.01), f"Failed for value {to_encode}"
